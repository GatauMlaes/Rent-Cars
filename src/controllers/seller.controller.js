...
BASE_URL.pathname += 'Kelompok-3/seller/change-password'
...
class Seller_Controller_Auth{
    ...
    async register (req,res,next) {
        try {
            ...
            res.redirect(`/Kelompok-3/seller/otp-verification/${input.email}`)
        } catch (error) {
            next(error);
            res.redirect("/Kelompok-3/seller")
    ...
    async otp_verification (req,res,next){
        ...
        try {
            ...
            next(error)
            res.redirect(`/Kelompok-3/seller/otp-verification/${email}`)
        }
    }
    async login (req,res,next) {
        try {
            ...
            res.redirect(`/Kelompok-3/seller/verification?accesToken=${result.accessToken}`)
        } catch (error) {
            next(error);
            res.redirect("/Kelompok-3/seller/login")
        }
    ...
    async forgot_password (req,res,next){
        try {
            ...
            res.redirect("/Kelompok-3/seller/forgot-password")
        } catch (error) {
            next(error)
            res.redirect("/Kelompok-3/seller/forgot-password")
        }
    ...
    async reset_password (req,res,next){
        ...
        try {
            ...
            next(error)
            res.redirect(`/Kelompok-3/seller/change-password/${email}/${token}`)
        }
    }
    ...
    async logout (req,res,next){
        try {
            ...
            await service.logout(res, refreshToken, "/Kelompok-3/seller/login")
        } catch (error) {
            next(error)
        }
    }
