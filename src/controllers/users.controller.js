...
class Users_Controller_Auth{
    ...
    async forgot_password (req,res,next){
        try {
            ...
            const url = envConfig.base_url+`:${envConfig.port}`+'/Kelompok-3/users/change-password'
    ...
    async reset_password (req,res,next){
        try {
            ...
            res.redirect("/Kelompok-3/users/login")
        } catch (error) {
            next(error)
        }
    }
    ...
    async logout (req,res,next){
        try {
            ...
            await service.logout(res,refreshToken,"/Kelompok-3/users/login")
        } catch (error) {
            next(error)
        }
    }
